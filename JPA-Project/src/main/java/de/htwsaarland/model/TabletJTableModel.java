...
public class TabletJTableModel extends AbstractTableModel {
	...
	public void getDBContents(){
		...
		for(int i = 0; i < entries.size(); ++i) {
			...
			data[i][0] = currentTablet.getHersteller();
			data[i][1] = currentTablet.getModell();
			data[i][2] = currentTablet.getBetriebssystem();
			data[i][3] = currentTablet.getDisplaygroesse();
			data[i][4] = currentTablet.getProzessor();
			data[i][5] = currentTablet.getSpeicher() + " GB";
			data[i][6] = currentTablet.getConnectionType();
			data[i][7] = currentTablet.getPreis() + " €";
